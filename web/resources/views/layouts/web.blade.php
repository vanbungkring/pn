...
<html lang="en">
  <head>
    ...
    <title>{{ $title . '- Partai NasDem' }}</title>
    @yield('meta')
    <!-- Responsive CSS -->
...
<!-- Global site tag (gtag.js) - Google Analytics -->
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-74361370-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-74361370-1');
</script>
    	...
    	<div class="kode_top_strip">
        	<div class="container">
                <div class="kode_cnt_info">
                    <ul>
                        <?php
                            $menus =  app('App\Models\Menu');
                            $mm = $menus->where('daerah_id', 41)
                            ->orderBy('id')
                            ->get();
                    
                        ?>
                        @foreach($mm as $mn) 
                        <li><a href="{{$mn->uri}}">{{ $mn->title }}</a></li>
                        @endforeach
                    </ul>
                </div>
